...
<script>
import { RetrieveCompany, UpdateCompany } from '@/api/provider/company'
import { mapState } from 'vuex'
import util from '@/libs/util.js'

export default {
  name: 'provider-certificationmanagement-index7',
  data () {
    return {
      total: 20,
      pageSize: 10,
  ...
  mounted () {
    ...
    // console.log('uuid:' + util.cookies.get('uuid'))
  },
  computed: {
  ...
  methods: {
    createCompanyTable: function (data) {
      RetrieveCompany(JSON.stringify({ 'user_id': util.cookies.get('uuid'), 'flag': '1' }))
        .then(async res => {
